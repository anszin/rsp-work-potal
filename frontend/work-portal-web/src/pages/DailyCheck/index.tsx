    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button onClick={() => setDateFilter('')}
            style={{ padding: '6px 12px', border: '1px solid #ddd', borderRadius: 6, background: dateFilter ? '#fff' : '#1976d2', color: dateFilter ? '#333' : '#fff', cursor: 'pointer', fontSize: 13 }}>
            전체
          </button>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4, gridColumn: '1 / -1' }}>
                ...
                <textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} rows={3}
                  style={{ padding: '8px', borderRadius: 4, border: '1px solid #ccc', resize: 'vertical', fontFamily: 'inherit', fontSize: 14 }} />
              </label>
            </div>
      ...
      {isLoading ? <p>로딩 중...</p> : (
        <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff', borderRadius: 8, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
          ...
          <tbody>
            {reports.length === 0 ? (
              <tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: '#999' }}>점검 데이터가 없습니다.</td></tr>
            ) : reports.map(r => (
              <tr key={r.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{ padding: '10px 14px', fontSize: 13, color: '#555', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {r.note ?? '-'}
                </td>
                <td style={{ padding: '10px 14px', fontSize: 13, color: '#666' }}>{r.reporter}</td>
